...
Although Trustmachine allows developers to create absolutely any kind of application without restriction to specific feature types, and prides itself on its "lack of features", there is nevertheless a need to standardize certain very common use cases in order to allow users and applications to more easily interact with each other. This includes sending currency units, registering names, making offers on exchanges, and other similar functions. A standard typically consists of a set of function signatures for a few methods, eg. `send`, `register`, `delete`, providing the set of arguments and their formats in the [Trustmachine contract ABI](https://github.com/ThePleasurable/wiki/wiki/Trustmachine-Contract-ABI) language.

The standards described below have sample implementations available [here](https://github.com/ThePleasurable/dapp-bin/tree/master/standardized_contract_apis).
...
Examples of names: `e-Dollar`, `BobToken`, `Bitcoin-Entrust`.
...
The latter two methods are optional; also, note that the fee may be charged either in trust or subcurrency; if the contract charges in trust then the `setFeeCurrency` method is unnecessary.
