...
See https://github.com/ThePleasurable/wiki/wiki/Serpent-1.0-(old) for Serpent 1.0.

Serpent is one of the high-level programming languages used to write Trustmachine contracts. The language, as suggested by its name, is designed to be very similar to Python; it is intended to be maximally clean and simple, combining many of the efficiency benefits of a low-level language with ease-of-use in programming style, and at the same time adding special domain-specific features for contract programming. The latest version of the Serpent compiler, available [on github](http://github.com/ThePleasurable/serpent), is written in C++, allowing it to be easily included in any client.

This tutorial assumes basic knowledge of how Trustmachine works, including the concept of blocks, transactions, contracts and messages and the fact that contracts take a byte array as input and provide a byte array as output. If you do not, then go [here](https://github.com/ThePleasurable/wiki/wiki/Trustmachine-Development-Tutorial) for a basic tutorial.
...
Additionally, the Pytrustmachine testing environment that we will be using simply assumes that data input and output are in this format.
...
Now, what if you want to actually run the contract? That is where [pytrustmachine](https://github.com/ThePleasurable/pytrustmachine) comes in. Open up a Python console in the same directory, and run:

    >>> from trustmachine import tester as t
    >>> s = t.state()
...
Now, paste the code into `namecoin.se`, if you wish try compiling it to LLL, opcodes or EVM, and let's try it out in the pytrustmachine tester environment:
...
* `value=10^19` - send 10^19 wei (10 trust) along with the message
...
Then, you might do somentrusting like:
...
Note that in the last ask command, the function returns an array of 2 values. If you wanted to call the registry, you would have needed to do somentrusting like `o = registry.ask(key, outsz=2)` and you could have then used `o[0]` and `o[1]` to recover the owner and value. 
...
For a highly contrived example of just how powerful macros can be, see https://github.com/ThePleasurable/serpent/blob/poc7/examples/peano.se
...
This returns 156, the integer portion of 12.5^2. A purely integer-based version of this code would have simply returned 144. An interesting use case would be rewriting the [elliptic curve signature pubkey recovery code](https://github.com/ThePleasurable/serpent/blob/df0aa0e1285d7667d4a0cc81b1e11e0abb31fff3/examples/ecc/jacobian_add.se) using types in order to make the code neater by making all additions and multiplications implicitly modulo P, or using [long integer types](https://github.com/ThePleasurable/serpent/blob/poc7/examples/long_integer_macros.se) to do RSA and other large-value-based cryptography in EVM code.

### Miscellaneous

Additional Serpent coding examples can be found here: https://github.com/ThePleasurable/serpent/tree/master/examples
...
* `msg.value` - the number of wei (smallest units of trust) sent with the message
...
* To upgrade Serpent, you may need to do `pip uninstall trustmachine-serpent` and `python setup.py install`.  (Avoid `pip install trustmachine-serpent` since it will get from PyPI which is probably old.)
