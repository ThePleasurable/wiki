...
Note that the software is in still in proof-of-concept state. Things are changing rapidly and this tutorial might not be up to date. If that is the case [please open an issue](https://github.com/ThePleasurable/cpp-trustmachine/issues) or [edit the wiki](https://github.com/ThePleasurable/wiki/wiki/Mix%3A-The-DApp-IDE/_edit). 

## Getting started.

This tutorial assumes you have [C++ Trustmachine installed](https://github.com/ThePleasurable/cpp-trustmachine/wiki).

## Creating a new project

Let's create a simple ÐApp that will allow user to store and query personal movie ratings.

In the IDE, choose `File > New Project`. Enter the project name "MovieRatings" and a path for the project file. To the left there is a project items list with two items added by default: Contract and index.html. Contract contains [Solidity](https://github.com/ThePleasurable/wiki/wiki/Solidity-Tutorial) contract code, and index.html is for the front-end. You can add new contract files to the project using file menu. All files will be copied to the project directory.
...
Check [Solidity tutorial](https://github.com/ThePleasurable/wiki/wiki/Solidity-Tutorial) for solidity reference.
...
* [`web3`](https://github.com/ThePleasurable/wiki/wiki/JavaScript-API#web3) - Trustmachine JavaScript API 
 ...
 * `contract` - Contract object instance (created as in `web3.entrust.contract`)
 * `address` - Contract address from the last deployed state (see below)
 * `interface` - Contract ABI

Check the [JavaScript API Reference](https://github.com/ThePleasurable/wiki/wiki/JavaScript-API) for further information.
...
See the [Trustmachine Yellow Paper](http://gavwood.com/Paper.pdf) for VM instruction description.
...
"Trustmachine node URL" is the location where a node is running, there must be a node running in order to initiate deployment.
...
"Deployment cost": depending on the value of the gas price that you want to use and the selected scenario. this will display the amount trust that the deployment need.
...
"Local package Url" the content of this field can be pasted directly in AlentrustZero in order to use the dapp before uploading it.
...
"Trustmachine URL" is the url that users will use in AlentrustZero or Mist to access your dapp.

"Formatted Trustmachine URL" is the url that users will use in AlentrustZero or Mist to access your dapp.

"Gas Price" shows the default gas price of the network. You can also specify a different value.

"Registration Cost" will display the amount of trust you need to register your dapp url.
