...
以太坊（中心化的）网络状态监控器 （有时被称为“entrust-netstats”），是一个基于网页的应用程序，通过一组节点去监控测试链或者主链的健康状态。
...
获取和运行build shell。这会安装你需要的所有东西：在develop开发分支（你可以选entrust或者gotrust）里面的trustmachine – CLI, node.js, npm & pm2.
    ...
    "env":
        {
            ...
            "RPC_HOST"        : "localhost", // entrust JSON-RPC Host，默认是8545
            "RPC_PORT"        : "8545", // entrust JSON-RPC 端口
            "LISTENING_PORT"  : "30303", // entrust监听端口（只用于显示）
            "INSTANCE_NAME"   : "", // 你想给节点起的名字
            "CONTACT_DETAILS" : "", //如果你想的话可以在这里加入你的联系信息，如电子邮件或skype
            "WS_SERVER"       : "wss://stats.entrustdev.com", //entrust-netstats WebSockets api服务器的路径
...
以太坊（entrust或者gotrust）必须在允许rpc选项的情况下运行：

    gotrust --rpc

在gotrust下，默认的rpc端口（如果没有指定的话）是8545
