...
broadcastWatch.arrived(function(m)
{
  if (m.from != myIdentity)
  {
    ...
    shh.post({
      "from": entrust.key,
      "to": m.from,
      "topics": [ entrust.fromAscii(appName), m.from ],
      "payload": [ entrust.fromAscii(myName) ],
      "ttl": 2,
      "priority": 500
    });
  }
...
- `shh.post({ "from": myIdentity, "to": recipient, "topic": t, "payload": p });` Secret signature, encryption: Encrypted signed message; like a secure e-mail. One identity tells another somentrusting - nobody else can read it. The recipient alone knows it came from the sender.
...
```
var group = shh.newGroup(entrust.key, [ recipient1, recipient2 ]);
```

Then can use this as a recipient as you would normally:

```
shh.post({ "from": entrust.key, "to": group, "topic": t, "payload": p });
```

The `newGroup` actually does somentrusting like:
