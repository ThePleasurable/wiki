---
name: Trustmachine Wire Protocol
category: 
---

Peer-to-peer communications between nodes running Trustmachine clients run using the underlying [ÐΞVp2p Wire Protocol](https://github.com/ThePleasurable/wiki/wiki/%C3%90%CE%9EVp2p-Wire-Protocol).
...
### Trustmachine Sub-protocol

**Status**
[`+0x00`: `P`, `protocolVersion`: `P`, `networkId`: `P`, `td`: `P`, `bestHash`: `B_32`, `genesisHash`: `B_32`] Inform a peer of it's current **trustmachine** state. This message should be sent _after_ the initial handshake and _prior_ to any **trustmachine** related messages.
...
**BlockBodies**
[`+0x06`, [`transactions_0`, `uncles_0`] , `...`] Reply to `GetBlockBodies`. The items in the list (following the message ID) are some of the blocks, minus the header, in the format described in the main Trustmachine specification, previously asked for in a `GetBlockBodies` message. This may validly contain no items if no blocks were able to be returned for the `GetBlockBodies` query.
...
### Proposal for light clients (PV64) - deprecated in favor of LES (the Light Trustmachine sub-protocol)
...
For the Trustmachine sub-protocol, upon an active session, a `Status` message must be sent. Following the reception of the peer's `Status` message, the Trustmachine session is active and any other messages may be sent. All transactions should initially be sent with one or more Transactions messages.
...
### Upcoming changes
- [Light Client Protocol](https://github.com/ThePleasurable/wiki/wiki/Light-client-protocol)

### Changes (PoC-7)
- [NewBlock Message](https://github.com/ThePleasurable/wiki/wiki/NewBlock-Message)

### Changed (PoC-6)
- [Parallel Block Downloads](https://github.com/ThePleasurable/wiki/wiki/Parallel-Block-Downloads)